...
There are **8 schemas total**: 4 collections + 4 singletons (`showcaseItem`, `smartTrackingCards`, and `officeLocations` were added later as explicit scope changes — see `docs/DECISIONS.md`).
...
## 7. `officeLocations` — Singleton with repeater

Appears on: "Our Offices" band above the footer — home, services, services/[slug], /why-choose-us, /team, /showcase, /quote.
...
```
{
  offices: [
    {
      officeId: string,   // short unique slug, e.g. "hk", "usa"
      label: string,       // e.g. "Heli Skycargo Limited"
      country: string,     // e.g. "Hong Kong"
      address: string,
      phone: string,
      email: string,
      cityscape: image,    // background photo when this office is active
      order: number
    },
    ...
  ]
...
- All fields required except `order`
- `email` must be a valid email address

**Fallback**: when the doc is empty, or an entry is missing a required field or its cityscape image, the frontend renders the hardcoded `OFFICES` constant from `src/lib/constants.ts` instead (whole-array fallback, not per-entry).
...
| `modal_media`                | (image or videoSlide)[] (max 10)                                                  | Mixed popup carousel slides, editor-ordered; videoSlide = {url, poster?}  |
| `media_photos` / `video_url` | deprecated                                                                        | Superseded by `modal_media` (2026-07); legacy docs still render           |
| `gallery_images`             | (galleryImage {image, caption} or galleryVideo {url, poster?, caption})[] (max 8) | Thumbnail strip in popup; videos play in the main viewer on tap (2026-07) |
