...
**Currently working on**: M7 — Shipment Showcase complete (`/showcase` route static-generated with `revalidate: 60`). Sections: ShowcaseHero (full-bleed photo, uniform 0.36 overlay, **red** eyebrow, 2-line desktop / 3-line mobile H1, no subhead — Figma's mobile subhead was a stale smart-tracking carry-over), refactored `_shared/ProjectsMosaic` (now a single client component used by home + service-detail + /showcase; column-based masonry driven by per-tile `desktopColumn`/`mobileColumn`/`shape`, per-breakpoint Load More pills — desktop 8→14 single-shot vs mobile 4→8→12→14 batched cadence — owns the modal + photo-carousel state), new `_shared/ShowcaseModal` (split layout: photo left 50% / content right 50% on `lg+`, photo top + stacked content on mobile; ArrowLeft/Right keyboard nav, ESC + backdrop-click + close-X all close, body scroll-lock, smart "Request Quote" pill that scrolls to `#request-quote` on the current page or falls back to `/quote`), reused `QuoteFormShell` (with `#request-quote` anchor) + `<OfficesGlobal defaultActive="usa" />`. `ui/Modal.tsx` extended with `size?: "sm" | "lg" | "xl"` + `bare?: boolean` props (defaults preserve M2 behavior). `SHOWCASE_TILES` extended from 8 → 14 entries with `shape`, `desktopColumn`, `mobileColumn`, `photos`, `videoUrl`, `hasPlayIcon`, `modal.title` fields; legacy `showFlag` Japan-flag overlay replaced site-wide by `hasPlayIcon` + `/showcase/icon-play.svg` (Figma intent was always a play icon — see DECISIONS.md). New constants `SHOWCASE_HERO` + `SHOWCASE_GALLERY`. Home + service-detail pass `tiles={SHOWCASE_TILES.slice(0, 8)}` so the visible 8-tile output matches pre-M7 (only behaviour change: tiles are now clickable + Japan tile shows red play circle). `pnpm typecheck` + `pnpm lint` clean; production build succeeds (14 static pages); verified at 600/768/1024/1440/1920 in browser; mobile cadence 4 → 8 → 12 → 14 confirmed; modal opens from any tile on home + /showcase. Next: M8 — Request a Quote (multi-step form + Formspree wiring).
